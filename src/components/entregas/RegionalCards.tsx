...
export const RegionalCards = ({ data }: RegionalCardsProps) => {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
      {data.map((item) => {
        ...
        return (
          <Card key={item.id} className="bg-dashboard-card border-dashboard-border hover:border-dashboard-accent/50 transition-colors cursor-pointer">
            <CardContent className="p-3">
              <div className="flex items-center gap-2 mb-3">
                <MapPin className="h-4 w-4 text-dashboard-accent" />
                <span className="text-sm font-medium text-foreground truncate">{item.regional}</span>
              </div>
